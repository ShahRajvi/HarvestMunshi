...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (potIdInput) {
    ...
    potIdInput.addEventListener('input',
      (e) => {
        const inputValue = e.target.value;
        const parsedValue = parseInt(inputValue);
        const isDeletion = e.inputType === 'deleteContentBackward' || e.inputType === 'deleteContentForward';

        if (isDeletion) {
          // Allow deletion
          potIdInput.dataset.previousValue = inputValue; // Update previous value on deletion
          return;
        }
        if (e.data.includes('.')) {
          e.preventDefault();
          potIdInput.value.replace(/\./g, '');
          potIdInput.value = potIdInput.dataset.previousValue;
          potIdInput.focus();
          return;
        }
        ...
        if (isNaN(parsedValue) || parsedValue < 0) {
          ...
          potIdInput.dataset.previousValue = inputValue;
        }
      });
